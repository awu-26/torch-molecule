...
- Supports checkpoint caching for testing
"""
...
def run_quick_benchmark(resume=True, force_restart=False):
            ...
            if "GNNMolecularPredictor" in results["models_tested"]:
                ...
                if checkpoint and resume and not force_restart:
                    ...
                    try:
                        ...
                        print(f'Data shapes and types:')
                        print(f'  trainX: {type(trainX)} with {len(trainX)} samples')
                        print(f'  trainY: {type(trainY)} shape {trainY.shape if hasattr(trainY, "shape") else len(trainY)}')
                        print(f'  valX: {type(valX)} with {len(valX)} samples')
                        print(f'  valY: {type(valY)} shape {valY.shape if hasattr(valY, "shape") else len(valY)}')
                        print(f'  testX: {type(testX)} with {len(testX)} samples')
                        print(f'  testY: {type(testY)} shape {testY.shape if hasattr(testY, "shape") else len(testY)}')
                        print(f'  Sample trainY values: {trainY[:3] if len(trainY) >= 3 else trainY}')
                        print(f'  Sample testY type: {type(testY[0]) if len(testY) > 0 else "empty"}')
    ...
    # Save results
    output_file = Path(__file__).parent / f"quick_benchmark_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def main():
    ...
    run_quick_benchmark(resume=resume, force_restart=force_restart)
